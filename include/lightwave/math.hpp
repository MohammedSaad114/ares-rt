...
/// @brief 4 * Pi
static constexpr float FourPi = 12.56637061435917295384f;
/// @brief 1 / Pi
...
/// @brief Returns the cosine value of an angle in radians @c v .
inline float cos(float v) { return std::cos(v); }
/// @brief Returns the sine value of an angle in radians @c v .
inline float sin(float v) { return std::sin(v); }
/// @brief Returns the angle in radians for a cosine value @c v in (-1, +1)
inline float acos(float v) { return std::acos(v); }
/// @brief Returns the angle in radians for a sine value @c v in (-1, +1)
inline float asin(float v) { return std::asin(v); }

/// @brief Returns the value @c base raised to the power @c exponent .
inline float pow(float base, float exponent) {
    return std::pow(base, exponent);
}
/// @brief Evaluates the exponential function for the value @c v .
inline float exp(float v) { return std::exp(v); }
/// @brief Evaluates the natural logarithm for the value @c v .
 ...
 */
inline float safe_sqrt(float v) { return v <= 0 ? 0 : sqrt(v); }
/**
 ...
 */
inline float safe_acos(float v) { return acos(clamp(v, -1.f, +1.f)); }
...
public:
    ...
    /// @brief Constructs an matrix filled with zeros.
    static auto zeros() { TMatrix BUILD2(0) }
};
...
/// @brief Returns true iff the given value is finite (i.e., no NaNs or
/// infinity).
template <typename T> inline bool check_finite(const T &n) {
    return std::isfinite(n);
}

/// @brief Returns true iff a given vector is (approximately) normalized.
template <typename T> inline bool check_normalized(const T &n) {
    const float lenSqr = n.lengthSquared();
    return abs(lenSqr - 1) < 1e-3f;
}

/// @brief Returns true iff two given vectors are (approximately) orthogonal.
